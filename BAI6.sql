...
FROM 
    Guests
JOIN 
    Bookings ON Guests.GuestID = Bookings.GuestID
JOIN 
...
FROM 
    Rooms
JOIN 
    Bookings ON Rooms.RoomID = Bookings.RoomID
JOIN 
    Guests ON Bookings.GuestID = Guests.GuestID
WHERE 
    Guests.Email IS NOT NULL 
    AND Guests.Email <> ''
ORDER BY 
    Rooms.RoomType ASC, 
    Rooms.RoomNumber ASC
LIMIT 5;
...
WHERE 
    Guests.Gender != 'Nu'
GROUP BY 
    Guests.FullName, Guests.Email
HAVING 
    COUNT(Bookings.BookingID) >= 2;
